...
*   **L1 + Phase 1.5 全面加固 (V1.7, 2026-06-01, L1 收尾)**：V1.6 模式扩展到 Phase 1.5 三个 skill（`risk_analyzer` / `scenario_extractor` / `session_summary`），后者两个从裸 `llm.ainvoke().content` 切换到 `safe_structured_invoke`。修复 L1 验证报告 3 个盲点：A（互斥性断言）确认非盲点已存在；B（`unknown_actor` 无下游消费）修了——`CoverageReport` 新增 `unknown_actor_count` + `unknown_actor_names` + HTML 报告 L1 卡片新增"未匹配角色"折叠区；C（fast-path 90% 阈值）加 8 行 docstring 说明依据。新增 24 个 Phase 1.5 回归测试（20 mock + 4 live skip）。**L1 + Phase 1.5 8 个 skill 全部统一为 V1.6 5 段 XML 模式，53 mock + 8 live skip 回归测试全过，L1 收尾完成**（详见 `docs/l1-verification-report-v1.7.md` + `docs/devlog/20-layer1-prompt-engineering-v17.md`）。下一步 P0：把 V1.6 模式迁移到 L2 `execution_graph.py`（prompt 调用的 hot path，每步 1 次）。
*   **N2 SystemModel 三层防御加固 (V1.6.1 / Phase 1.6.1, 2026-06-02, V2.0 v2 先头兵)**：V1.7 报告原文点名的"N2 fallback 触发 3/4 fixture"P0 漏洞修复。**3 层防御**：(1) Prompt 加固——`core/skills/system_modeler.py` 加 rule 6 (transitions 去重) + 5 条自检步骤；(2) 代码层 normalize——新增 `_strip_node_suffix` (剥 5 后缀: 状态/流程/页/中/期) + `_align_action` (substring 双向对齐到 use_case.name) + `_normalize_system_model` (后处理, 自动修 LLM 输出违规) + `_is_chinese_noun_phrase` 收紧 (与生产代码 import 共享, 防止漂移); (3) 兜底——新增 `_derive_minimal_system_model(ucm)`, LLM 完全失败时返回 UseCaseModel 推导骨架, **不再返回空 SystemModel()**。4 fixture live 验证: **0 invariant violations** (was 1+ fixture 有"状态"后缀 + 1 fixture transitions 重复); 0 minimal fallback triggered (但安全网已就位); inner fallback 仍 3/4 (mimo-v2.5 SDK 行为, V1.6.1 范围外, V2.1+ 改 SDK/换端点再说)。回归测试: **60 mock passed + 8 live skipped in 0.71s** (新增 7 个 V1.6.1 测试)。**Phase 1.6.1 落地, N2 SystemModel 从 L1 最薄弱节点升级为 L1 最强节点 (3 层防御 + 0 violation)**。详细: `docs/devlog/22-phase16-completion.md` + `data/n2_v161_live_results.json` + `data/n2_fallback_debug.json` (V1.7 baseline 对比)。
*   **planning_graph explore V1.6 化 + SystemMap 采样加固 (V1.6.2/1.6.3/1.6.4 / Phase 1.6, 2026-06-02, V2.0 v2 L1 收尾)**：V2.0 v2 计划 §3.0 的 Phase 1.6 全部完成。**3 件套加固**：(1) `agents/ui/prompts.py` 的 `get_exploration_system_prompt` 重写为 V1.6 5 段 XML (role/context/task/rules/examples/output_contract), `<output_contract>` 显式约束 "tool_call 必填 OR 显式 stop", 杜绝纯文本死循环; (2) `agents/ui/planning_graph.py` 的 `explore_decide_node` 显式注入 `task_config._system_model` 的 system_name/modules/entities/flow_names 作为"理论导航地图" (V1.7 漏点: LLM 探索时不知道理论业务模块, 易漏核心功能区), `explore_execute_node` docstring 标注 inter-node 契约 (工具失败返 ToolMessage 不抛异常); (3) `core/skills/system_mapper.py` 完整重写: 采样 10/15 → **20/30** (env 可降级 `SYSTEM_MAP_MAX_PAGES` / `SYSTEM_MAP_MAX_ELEMENTS_PER_PAGE`), V1.6 5 段 XML prompt + few-shot (good/bad), **双入口** `extract_system_map_structured() -> SystemMap` (pydantic 强类型) + `generate_system_map() -> dict` (兼容层)。`docs/prompt-engineering.md` §8 升级为完整 §9 (planning_graph 探索子图契约: 节点定位 / V1.6 模板 / inter-node 契约表 / 5 个改造点 vs V1.7 漏点 / 8 个反模式 / 27 个回归测试)。**Best practice 调研 7 篇**: Anthropic 2026 prompt engineering + Context Engineering 2025-09 + Writing Tools for Agents 2025-09 + ReAct (Yao 2022) + Codebridge 2026 Sub-agent Manifest + AppScale 2026 Structured Output + LangGraph 2026 Production Best Practices。**Live 验证**: `SYSTEM_MAP_LIVE=1` 跑 20 页探索历史 → 5 pages / 17 actions / 2 forms in 16.78s。**回归测试: 97 mock passed + 9 live skipped in 27.30s** (新增 27 个: 13 V1.6.2 + 14 V1.6.3)。**L1 收尾完成**: 8 L1 skill + 3 Phase 1.5 skill + planning_graph explore + system_mapper 全部 V1.6 化, 10 个 V1.6 5 段 XML prompt。详细: `docs/devlog/23-phase16-2-3-4-completion.md` + §9 planning_graph 探索子图契约。下一步: Phase A (L2 安全网 + 测试基础设施, 1.5d)。
