...
async def _get_db_id(task_id: str) -> int:
    """Resolve task_id (UUID or DB ID string) to internal database auto-increment integer ID."""
    ...
    try:
        ...
        async with async_session() as session:
            ...
            if task is not None:
                _task_id_map[task_id] = db_id
                return db_id
    except ValueError:
        pass

    async with async_session() as session:
        stmt = select(Task).where(Task.config['task_id'].astext == task_id)
...
async def log_test_plan(task_id: str, test_plan: list[TestCase]) -> None:
    ...
    """
    db_id = await _get_db_id(task_id)
    ...
    async with async_session() as session:
        ...
        if step.screenshot_path and len(step.screenshot_path) > 1000:  # heuristic for base64
            try:
                ...
                import logging
                logging.warning(f"Failed to save screenshot to disk: {e}")
...
async def log_test_result(task_id: str, result: TestResult) -> None:
    ...
    and persist retry metadata (retry_count, failure_context, status) to task_step.
    ...
    async with async_session() as session:
        ...
        if result.status == "passed":
            task.passed_tests += 1
        elif result.status in ("failed", "human_review_required", "incomplete"):
            task.failed_tests += 1
        # "skipped" does not increment either counter

        # Persist retry metadata to the last step of this test case
        from sqlalchemy import select, desc
        stmt = (
            select(TaskStep)
            .where(TaskStep.task_id == db_id, TaskStep.test_case_id == result.test_case_id)
            .order_by(desc(TaskStep.id))
            .limit(1)
        )
        row = await session.execute(stmt)
        last_step = row.scalar_one_or_none()
        if last_step:
            last_step.test_case_status = result.status
            last_step.retry_count = result.retry_count
            last_step.failure_context = result.failure_context if result.failure_context else None
...
async def clear_test_case_steps(task_id: str, test_case_id: str) -> None:
    """Clear all task steps for a specific test case of a task."""
    db_id = await _get_db_id(task_id)

    async with async_session() as session:
        from sqlalchemy import delete
        stmt = delete(TaskStep).where(TaskStep.task_id == db_id, TaskStep.test_case_id == test_case_id)
